...
let getCommonStyle = function (styleObj, scalingRatio = 1) {
	...
	style.backgroundImage = style.backgroundImage ? `url(${   (style.backgroundImage.includes("http") || window.location.href.includes(":4000")) ? style.backgroundImage : '/h5-template' +style.backgroundImage})` : '';
	style.whiteSpace = "break-spaces"
	return style;
}


let getCommonStylePercent = function (styleObj, scalingRatio = 1) {
	let needUnitStr = ['width','top', 'left',  'paddingTop', 'paddingLeft', 'paddingRight', 'paddingBottom', 'marginTop', 'marginLeft', 'marginRight', 'marginBottom', 'borderWidth','fontSize', 'borderRadius', 'letterSpacing']
	let needUnitPercent = ["height"]
	let style ={}
